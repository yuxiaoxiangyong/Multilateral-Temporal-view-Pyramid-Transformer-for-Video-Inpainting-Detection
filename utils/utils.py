...
def train_log_dump(args, map_loss_sum, mani_lss_sum, natu_lss_sum, iou_lss_sum, focal_lss_sum,
    ...
    print(f'[Epoch: {epoch+1}, Step: {step + 1}] iou1: {iou_loss:.3f} iou2: {iou_loss1:.3f} map1: {loc_map_loss:.3f} mani1: {manipul_loss:.3f} natural1: {natural_loss:.3f} map2: {loc_map_loss1:.3f} mani2: {manipul_loss1:.3f} natural2: {natural_loss1:.3f}')
...
def get_optimizer(optim_name, lr, parameters, weight_decay = 1e-4, momentum = 0.9): # 添加优化器
    if optim_name == 'sgd':
...
def save_checkpoint(args, encoder, decoder, enc_opt, dec_opt, epoch=None):
    if epoch is not None:
        # print(os.getcwd())
        torch.save(encoder.state_dict(), os.path.join('../src/models',args.model_name,'encoder_{:}.pt'.format(epoch)))
        torch.save(decoder.state_dict(), os.path.join('../src/models',args.model_name,'decoder_{:}.pt'.format(epoch)))
        torch.save(enc_opt.state_dict(), os.path.join('../src/models',args.model_name,'enc_opt_{:}.pt'.format(epoch)))
        torch.save(dec_opt.state_dict(), os.path.join('../src/models',args.model_name,'dec_opt_{:}.pt'.format(epoch)))
    else:
        # print(os.getcwd())
        torch.save(encoder.state_dict(), os.path.join('../src/models',args.model_name,'encoder.pt'))
        torch.save(decoder.state_dict(), os.path.join('../src/models',args.model_name,'decoder.pt'))
        torch.save(enc_opt.state_dict(), os.path.join('../src/models',args.model_name,'enc_opt.pt'))
        torch.save(dec_opt.state_dict(), os.path.join('../src/models',args.model_name,'dec_opt.pt'))        
    # save parameters for future use
    pickle.dump(args, open(os.path.join('../src/models',args.model_name,'args.pkl'),'wb'))
...
def save_checkpoint_prev_inference_mask(args, encoder, decoder, enc_opt, dec_opt):
    torch.save(encoder.state_dict(), os.path.join('../src/models',args.model_name + '_prev_inference_mask','encoder.pt'))
    torch.save(decoder.state_dict(), os.path.join('../src/models',args.model_name + '_prev_inference_mask','decoder.pt'))
    torch.save(enc_opt.state_dict(), os.path.join('../src/models',args.model_name + '_prev_inference_mask','enc_opt.pt'))
    torch.save(dec_opt.state_dict(), os.path.join('../src/models',args.model_name + '_prev_inference_mask','dec_opt.pt'))
    # save parameters for future use
    pickle.dump(args, open(os.path.join('../src/models',args.model_name + '_prev_inference_mask','args.pkl'),'wb'))

def load_checkpoint(model_name,use_gpu=True,epoch=8):# 加载预训练的模型
    print(f'load checkpoints {epoch}')
    if use_gpu:
        encoder_dict = torch.load(os.path.join('../src/models',model_name,f'encoder_{epoch}.pt'))
        decoder_dict = torch.load(os.path.join('../src/models',model_name,f'decoder_{epoch}.pt'))
        enc_opt_dict = torch.load(os.path.join('../src/models',model_name,f'enc_opt_{epoch}.pt'))
        dec_opt_dict = torch.load(os.path.join('../src/models',model_name,f'dec_opt_{epoch}.pt'))
    else:
        encoder_dict = torch.load(os.path.join('../src/models',model_name,f'encoder_{epoch}.pt'), map_location=lambda storage, location: storage)
        decoder_dict = torch.load(os.path.join('../src/models',model_name,f'decoder_{epoch}.pt'), map_location=lambda storage, location: storage)
        enc_opt_dict = torch.load(os.path.join('../src/models',model_name,f'enc_opt_{epoch}.pt'), map_location=lambda storage, location: storage)
        dec_opt_dict = torch.load(os.path.join('../src/models',model_name,f'dec_opt_{epoch}.pt'), map_location=lambda storage, location: storage)

    # save parameters for future use
    args = pickle.load(open(os.path.join('../src/models',model_name,'args.pkl'),'rb'))
