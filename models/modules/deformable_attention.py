...
class DAttention(nn.Module):
    ...
    def forward(self, x1, x2, return_attention=False):
        # x1 x2 ==> B * N * C
        base = int(56 * math.pow(2, -self.cur_stage))
        x1 = rearrange(x1, 'b (h w) c -> b c h w', h = base * self.heigth_scale[0], w = base)
        x2 = rearrange(x2, 'b (h w) c -> b c h w', h = base * self.heigth_scale[1], w = base)
        B, C, H, W = x1.size()
        ...
        n_sample = Hk * Wk
        if self.offset_range_factor > 0: # 缩放offset数值
        ...
        #my_points = torch.clamp_(pos[..., (1, 0)]+1, min=0, max=2)
        x_sampled = F.grid_sample(
            input=x2.reshape(B * self.n_groups, self.n_group_channels, H * self.heigth_scale[1], W), 
        ...
        #temp = torch.sum(torch.mean(attn, dim=0), dim=0) # 所有patch的attention的均值
        
        #temp = torch.sum(attn[:,120,:], dim=0)
        #top_values, top_indexes = torch.topk(temp, 10)
        x = (attn @ v).transpose(1, 2).reshape(B, C, H, W) # N1 * N2 x N2 * C ==> N1 * C
        attn = attn.reshape(B, -1, attn.shape[-2], attn.shape[-1])

        #my_points = my_points.reshape(my_points.shape[0], -1, my_points.shape[-1])
        #my_points = my_points[:, top_indexes, :]
...
class SwinDAttention(nn.Module):
    ...
    def forward(self, x1, x2, return_attention=False):
        # x1 x2 ==> B * N * C
        x1 = rearrange(x1, 'b (h w) c -> b c h w', h = 7, w = 7)
